...
3. SC_BUS_ADC 达到 10.5 V 后，LED4 立即常亮，并继续保持充电 5 s。
...
| --- | --- |
| 小于或等于 4.5 V | 熄灭 |
| 4.5 V～10.5 V | 固定 1 s 周期，占空比等于估算电量百分比 |
| 大于或等于 10.5 V | 常亮，包括充满后的 5 s 保持充电阶段 |

电量百分比在 4.5 V～10.5 V 之间按电压线性估算：4.5 V 对应 0%，10.5 V
对应 100%。例如估算电量为 20% 时，每个 1 s 周期点亮 200 ms；估算电量为
80% 时点亮 800 ms。周期始终保持 1 s，仅改变点亮时间，因此可以直接通过
亮灭比例判断剩余储能。
...
典型输出如下：
    ...
    SPM input=12.031V bus=11.982V sc=10.512V mode=NORMAL boost=1 charge=0
    output=1 force_boost_off=0 force_output_off=0 faults=0x00 muted=0 counts=2/1
...
4. SC 达到 10.5 V 时确认 LED4 立即常亮。
5. 从首次达到 10.5 V 开始计时，连续 5 s 后充电器应关闭。

如果保持期间 SC 重新低于 10.5 V，5 s 计时会清零并重新开始。
充电过程中 LED4 的周期应始终约为 1 s，随着 SC 电压升高，每个周期内的
点亮时间应逐渐增加，达到 10.5 V 后转为常亮。
...
5. 恢复外部输入后，确认系统回到 mode=2，BOOST 仍保持开启。
...
4. 故障恢复后再次制造故障，蜂鸣器必须重新报警。
5. 对照 faults 检查故障位，并确认 EEPROM counts 在新故障或自动备电事件后
   增加。
...
- LED4 的电量显示周期固定为 1 s，点亮时间按 4.5 V～10.5 V 线性映射。
